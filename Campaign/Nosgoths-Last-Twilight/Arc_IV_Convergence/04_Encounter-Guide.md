# Arc IV Encounter Guide

Use this document to plan Convergence session by session. The finale is too variable for a single fixed enemy ladder, so each session lists the most usable encounter packages by purpose.

Only Session 4 is meant to default to a true full-scale battlefield encounter. Sessions 1-3 and 5 are consequence or pressure scenes unless you intentionally reinforce them.
...
- Package selection: `unified-pending` usually wants the Fracture package testing the center. `contested-opening` usually wants the Sarafan package exploiting the missing anchor. `internally-fractured-but-functioning` should use whichever faction can peel the visibly diverging member. `merel-advised` should use whichever faction the party threatened in the private approach.
- Reinforced option: Add a second Inquisitor to the Sarafan package or a Fracture Void-Touched to the Fracture package only if Session 1 must carry a full combat by itself.
...
- Kain-side note: If the Kain contract is not `Open`, represent Kain-side pressure here through Sevran, rumor, or silence rather than a physical Kain sighting.
- Reinforced option: Add a Hylden Rift-Priest to the physical package, or 1 extra Silent Mourner to the metaphysical package, if you want the road itself to become a full confrontation.
...
- Core objective: Make chamber control and answer-shaping more important than kill count.
- Baseline use: Treat the Hylden and Fracture packages as fixed rosters for chamber pressure. Do not add further summoned or reinforcement bodies from the Rift-Priest or Void-Touched unless you are intentionally upgrading Session 3 into a true primary combat.
- Status framing: `black_fulcrum_status: cleared` means defend a recognized account. `conditional` means the opposition should attack the known incomplete portions. `unfounded` means the scene becomes a scramble for legitimacy without Hall protection. If the coalition record is `witness-anchored`, the current Last Witness should be part of what enemies can threaten, discredit, or force the party to protect.
- Reinforced option: If Session 3 must carry the main battle, pair the Hylden package with 1 additional Hylden Shock Trooper or shift directly to the Session 4 Pillar Heath package.

## Session 4: Pillar Heath Seal-Breaker Push

- Primary location: Pillar trench, pylon front, and operator high ground.
...
- Terrain notes: Failing ritual geometry, fleeing witnesses, and a world already changing around the party.
- Antagonist guidance: Use Elain if the Wheel was broken but she survived, a Fracture remnant if the Fracture still has operational doctrine, or a Hylden commander if the Gate was sealed by force and someone still wants revenge through the wound.
- Aftermath note: Read the matching First Light scene after this encounter or mercy beat, before any broader ledger of who won what.