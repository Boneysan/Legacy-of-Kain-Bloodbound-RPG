# 05 — Which Future Is Real

The Chronoplast showed two futures. The party has been walking through Arc III carrying the knowledge that both are possible, that neither eliminated the other, and that the Heart of Nosgoth cannot fulfil both at once. This scene does not resolve that contradiction. It asks the party to choose which future they will act as though is real — knowing they may be wrong — before Arc IV makes the choice for them in a form they can't revise.

---

## Scene Info

- **Arc / Session:** Arc III — Session 5 (closing scene, hard cap before Arc IV)
- **Trigger:** The party holds all the intelligence they will have from this arc: Chronoplast reading, Wheel exposure, betrayal resolved or unresolved, fragment secured. A decision must be made about what they believe.
- **Scene Type:** `reckoning` + `closing-declaration`
- **Est. Duration:** 5–8 min
- **Tags This Scene Can Change:** `party_operative_truth`, `chronoplast_access`, `chronal_shard_fate`

---

## Cast

| NPC | Role in This Scene |
|-----|--------------------|
| Black Marshal Sevran | The judgment-bringer; makes clear that the party's Arc IV position is determined by what they're willing to commit to, not by what they're certain of |
| Iriane Quell | Wants the party to pick the future they want rather than the future they fear, but won't say it directly until asked |
| Silk-Without-Moon | Final Arc III appearance; briefly. She is here to remind the party that she is building toward the future she chose long ago, and that the party's uncertainty is a gift to her |

---

## Opening Narration

> The Chronoplast chamber is not quite cold. The ice that ate the eastern corridor has retreated far enough that the records room is usable, and Iriane has spread what she organized into three stacks by category: things that can be verified by external record, things that can be verified only by the Shard, and things that cannot be verified at all. The largest stack is the third one. Black Marshal Sevran is consulting the map that shows where the coalition presently stands, and he is not pointing at anything. He is waiting for the party to arrive at the conclusion he already reached. The window that Silk-Without-Moon left through is still open. It is letting in cold air. Someone should probably close it.

---

## NPC Opening Line

> **Black Marshal Sevran**, without turning: *"Two of you have been treating the Chronoplast as an intelligence problem. One of you has been treating it as a spiritual problem. What I need to know — before we agree on anything about Arc IV — is whether the group can operate on a single version of events. Not the true version. Kain is not asking for the true version. He is asking whether the party can commit to one account of what they saw, act on it consistently, and stop revising it when it produces uncomfortable obligations."*

---

## Player Choices

### A — Commit to Future A (The Pillar Holds; the Heart Is Used as a Battery)

> The future in which the balance is preserved by sacrifice — the Heart completes the Pillar restoration at the cost of the party's long-term claim to the fragment.

**NPC Response (Sevran):**

> He turns, finally. *"Then we act on that. It means we go into Arc IV as servants of a structure the party may not survive as beneficiaries of."* He looks around the group. *"Understand: choosing this future means you are not rewarded by it. The balance does not produce victors. It produces continuation."* He makes a note on the map. *"I can work with that."*

> If Iriane is present: *"It's the honest choice for people who aren't certain they're right. You can revise a sacrifice. You can't revise a seizure."*

**Outcome:** Arc IV opens with the party operating as balance agents. Their coalition-building is oriented toward restoration, not control. Factions that want to own the outcome will oppose them. Kain's entry in Arc IV is as an ally with a price, not an obstacle.

**Tags:** `party_operative_truth: balance-through-sacrifice`, `chronoplast_access: full`

---

### B — Commit to Future B (The Gate Opens; Hylden Terms Are the Best Available)

> The future in which Moebian balance has already broken and the least-bad option is structured Hylden reintegration on terms the party can shape.

**NPC Response (Sevran):**

> A longer pause. *"That's the harder call. And it's the kind of choice that looks like treachery after the fact if you lose. You understand that."* He studies them. *"I do not disagree with it on principle. The Gate has been opening for three arcs. Calling it by its right name is not surrender — it's honesty. But you will have to say it out loud in Coalition Hall to the Sarafan, the Wheel, and the two surviving clans."* He makes a different note on the map. *"I'll be in the room."*

> Silk-Without-Moon, from near the window: *"I have spent a decade trying to get someone with standing to say that in public. I am — obscurely — moved."*

**Outcome:** Arc IV opens with the party as architects of a managed transition. Their coalition is smaller but more coherent. Anti-Hylden factions become active antagonists. The Glass Voice takes a stronger role in Arc IV negotiations. The ending available to this path includes the darkest win condition and the most coherent partial success.

**Tags:** `party_operative_truth: hylden-reintegration`, `chronoplast_access: full`

---

### C — Refuse to Choose; Carry Both Futures Into Arc IV as Live Variables

> The party will not collapse the ambiguity. They will operate with contingent strategies and adjust as new information arrives.

**NPC Response (Sevran):**

> He is quiet for a moment. *"That is not a choice I can support with the leverage I have. Kain does not fund equivocation."* He caps the map. *"I will be in Coalition Hall. I will stand with you as individuals. What I cannot do is commit resources to a position that has not decided what it is yet."* He pauses. *"Come back to me when you've finished deciding. I'll be there."*

> Silk-Without-Moon, already leaving: *"I'll be there too. We both know which future I'm building."*

**Outcome:** The party enters Arc IV without confirmed Kain-side backing. They are more agile politically but more exposed strategically. Their coalition choices in Arc IV become higher-stakes because they lack guaranteed support. The `party_operative_truth` tag records `undecided`, which activates a harder version of the coalition hall confrontation.

**Tags:** `party_operative_truth: undecided`, `chronoplast_access: partial`

---

### D — Ask Iriane Quell Which She Would Choose

> Not deferring — asking for the view of someone who has been watching time's accounting longer than any of them.

**NPC Response (Iriane):**

> She looks up from her stacks. A long pause. *"The future you should act as though is real is the one you are still trying to reach. Not the one you fear, not the one you're making your peace with. The one that would require you to be better than you currently know how to be."* She sets down the document she was holding. *"I'm aware that's not operational guidance. It's the only honest answer I have left."*

**After this, the party still must choose from A, B, or C.** But if they choose A or B immediately after Iriane's answer — if the answer moves them — the tag value gains a suffix: `party_operative_truth: balance-through-sacrifice (chosen under testimony)` or `party_operative_truth: hylden-reintegration (chosen under testimony)`. This has a small mechanical carry. In Arc IV Coalition Hall, Iriane will speak for them without being asked.

**Tags:** `party_operative_truth: balance-through-sacrifice (chosen under testimony)` — if Choice A follows; `party_operative_truth: hylden-reintegration (chosen under testimony)` — if Choice B follows; `party_operative_truth: undecided` — if Choice C follows

---

## If No Clear Choice Is Made

> If the party cannot reach consensus and the scene ends without a commitment, the `party_operative_truth` tag defaults to `unresolved`, which is treated as `undecided` for Arc IV purposes but does not deactivate Sevran — it delays him. He appears in Arc IV at Session 2 rather than Session 1. The party has cost themselves reaction time.

**Default Tag:** `party_operative_truth: unresolved`

---

## Arc III Close — Carry-Forward Summary

At the end of this scene, the Arc III state tracker should be finalized:

| Final Tag | Carry-Forward Effect in Arc IV |
|-----------|-------------------------------|
| `chronoplast_access` | `full` — party operates on their chosen account; `partial` — party is still revising; `denied` (if Vignette 01 outcome) — party has only partial Chronoplast intelligence |
| `chronal_shard_fate` | `recovered`, `destroyed`, `lost` — where the fragment is; modifies Heart assembly possibility in Arc IV |
| `wheel_exposure` | `broken` — Wheel enters Arc IV as combatant, not philosophy; `named` — Wheel is publicly accused but not dismantled; `concealed` — Wheel appears in Arc IV with all prior infrastructure intact |
| `party_operative_truth` | Freeform — Arc IV opens with the GM having this string and using it to anchor every faction's opening posture to the party |
| `betrayal_resolved` | `absorbed`/`severed`/`weaponized`/`unresolved` — determines Witness's Arc IV availability and behavior |

---

## GM Notes

- This is the last vignette of Arc III and the structural hinge of the entire campaign. Do not let it feel procedural. The party has earned an emotionally weighty conversation. Let it breathe.
- Sevran's exit from the scene if the party chooses `undecided` should feel like a real loss — not a punishment, but a consequence. He is not punishing them. He genuinely cannot work with undecided.
- Silk-Without-Moon's presence in this scene is minimal by design. She has already made her choice. She is present as a reminder that other people have been operating on committed accounts for a long time, and that certainty does not require being right — only being consistent.
- The suffix mechanic from Choice D (testimony-influenced decision) is a small reward for honest engagement with Iriane's answer. The GM should note it and honor it in Arc IV Coalition Hall without fanfare — just have Iriane stand up and speak.
- If `betrayal_resolved` is still `unresolved` when this scene closes, indicate it in the final state tracker. The Witness is going to Arc IV without resolution, and Warden Asha Merel will ask the party about it directly. That is not a trap. It is a test of whether the party can explain the wound in their coalition before the Hall is asked to trust it.
