# Nosgoth's Last Twilight Campaign Hub

This folder breaks the campaign into usable GM support files.
...
Arc IV also includes an endgame adjudication matrix for resolving Wheel, Hylden, Pillar, and faction fallout without rebuilding the ending logic from scratch.
...
## Current Major Fronts

- Hash'ak'gik cult infiltration around Avernus and refugee routes.
- Sarafan pressure on visibly corrupted or supernatural PCs, with the order increasingly split between legalist custody and Hollow purge logic.
- Clan competition over Dumah's Fortress and the right to define vampire rule.
- Hylden escalation near the Gate and Resonance Pillar.
...
- **Last Witness:** Update the current bearer at the end of each arc. The role belongs to the latest ordinary survivor who can still speak a fragment's cost plainly. **Tracked mechanically** as the `last_witness` tag in each Arc's VTT-State file (status: `unnamed` / `named` / `protected` / `exposed` / `lost`, plus bearer, what they witnessed, and forward fear). The role is first seeded in Arc I V05 (Fragment Custody), can change hands at Arc II V05 (Breach Decision), and is secured for the finale at Arc III V05 (Which Future Is Real). It **must arrive in Arc IV as `named` or `protected`** for the Arc IV V02 Choice D (`black_fulcrum_status: witness-anchored`) and the V05 Outcome D ending (The Dedication) to be reachable.
...
- **Balance Reckoning:** Use this as withdrawal of grace rather than Corruption by another name. Closed shrines, withheld rites, and routes that suddenly demand named cost are the right signals. **One device, two views of the same 0–4 clock:** the canonical advance rules live in [Faction-Tracker/Faction-Clocks.md](Faction-Tracker/Faction-Clocks.md#balance-reckoning-clock) (Arc-End Update Cheat Sheet), and each arc's VTT-State file mirrors the current value as the `balance_reckoning` tag — do not run a second counter on a different scale. It advances when the party spend a fragment to coerce an outcome (the clearest single hook is Arc II V05, where sealing or redirecting the breach ticks it while destroying the Ember or allowing failure does not). The value is rendered as withdrawal of grace on the Arc III road and read at the finale by the Arc IV Endgame Adjudication Matrix (3–4 hardens the Hall's judgment).
